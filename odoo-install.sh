...
#-------------------------------------------------------------------------------
# Using a root user or a user in the sudo group, Make a new file:
...
# Set to "True" to install certbot and have ssl enabled, "False" to use http (not recommended to disable due to security & encryption!)
ENABLE_SSL="True"
...
#--------------------------------------------------
echo -e "\n---- Install PostgreSQL Server ----"
if [ $INSTALL_POSTGRESQL_14 = "True" ]; then
...
# Installs RTL CSS to support RTL languages
...
EOF
  ...
  sudo mv ~/odoo /etc/nginx/sites-available/$WEBSITE_DOMAIN
  sudo ln -s /etc/nginx/sites-available/$WEBSITE_DOMAIN /etc/nginx/sites-enabled/$WEBSITE_DOMAIN
  sudo rm /etc/nginx/sites-enabled/default
  sudo service nginx reload
  sudo su root -c "printf 'proxy_mode = True\n' >> /etc/${OE_CONFIG}.conf"
  echo "Done! The Nginx server is up and running. Configuration can be found at /etc/nginx/sites-available/$WEBSITE_DOMAIN"
else
...
if [ $INSTALL_NGINX = "True" ] && [ $ENABLE_SSL = "True" ] && [ $ADMIN_EMAIL != "odoo@example.com" ]  && [ $WEBSITE_DOMAIN != "_" ];then
  sudo apt-get update -y
  sudo apt install snapd -y
  sudo snap install core; snap refresh core
  sudo snap install --classic certbot
  sudo apt-get install python3-certbot-nginx -y
  sudo certbot --nginx -d $WEBSITE_DOMAIN --noninteractive --agree-tos --email $ADMIN_EMAIL --redirect
  sudo service nginx reload
  ...
  if $WEBSITE_DOMAIN = "_";then
    echo "Website name is set as _. Cannot obtain SSL Certificate for _. You should use real website address."
  fi
fi

# Starting & Printing the server information
echo -e "* Starting Odoo Service"
sudo su root -c "/etc/init.d/$OE_CONFIG start"
echo "-----------------------------------------------------------"
echo "Done! Your Odoo server is up and running. Please save the following information:"
echo "Port: $OE_PORT"
...
echo "Superadmin Password (database): $OE_SUPERADMIN"
echo "To Start Odoo service: sudo service $OE_CONFIG start"
echo "To Stop Odoo service: sudo service $OE_CONFIG stop"
echo "To Restart Odoo service: sudo service $OE_CONFIG restart"
if [ $INSTALL_NGINX = "True" ]; then
  echo "Nginx configuration file: /etc/nginx/sites-available/$WEBSITE_DOMAIN"
fi
echo "-----------------------------------------------------------"